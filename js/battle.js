...
function getModBonuses(tankId){
  ...
  mods.forEach(m=>{if(m&&CONFIG.MODULES[m]){const mod=CONFIG.MODULES[m];b[mod.stat]=(b[mod.stat]||0)+mod.value;}});
  return b;
}
...
function getAllBonuses(tankId){
  const mod=getModBonuses(tankId);
  const upg=getUpgradeBonuses(tankId);
  return{mod,upg};
}

function startBattle(mode){
  GameState.gameActive=true;GameState.battleDmg=0;GameState.battleKills=0;
  GameState.curMap=document.getElementById('map-select').value;
  GameState.consumables=[false,false,false,false,false,false];
  GameState.adrenalineActive=false;GameState.fuelBoostActive=false;GameState.paiokActive=false;
  ...
  if(GameState.controlMode==='mobile')document.getElementById('mobile-controls').classList.add('show');
  else document.getElementById('mobile-controls').classList.remove('show');

  const bonuses=getAllBonuses(GameState.selected);
  GameState.player=new Tank(GameState.selected,-1500,0,'player',bonuses);
  GameState.units=[GameState.player];GameState.bullets=[];GameState.particles=[];GameState.tracks=[];
...
function startTraining(){
  ...
  const enemy=new Tank(eid,500,0,'enemy');enemy.angle=Math.PI;
  GameState.units=[GameState.player,enemy];GameState.bullets=[];GameState.particles=[];GameState.tracks=[];
  setupWalls(GameState.curMap);updateScoreboard();
}

function endBattle(won){
  GameState.gameActive=false;
  let silver=Math.floor(GameState.battleDmg*.5+GameState.battleKills*200);
  ...
  silver=Math.floor(silver*silverMul);xpReward=Math.floor(xpReward*xpMul);
  const goldReward=Math.floor((won?50:10)*goldMul);
  GameState.SILVER+=silver;GameState.XP+=xpReward;GameState.GOLD+=goldReward;
  if(GameState.quest23&&!GameState.quest23.claimed&&GameState.battleKills>0){GameState.quest23.kills+=GameState.battleKills;saveProgress();}
  ...
  document.getElementById('result-stats').innerHTML='Урон: '+Math.floor(GameState.battleDmg)+'<br>Фрагов: '+GameState.battleKills+'<br>Серебро: +'+silver+'₽<br>Опыт: +'+xpReward+' XP<br>Золото: +'+goldReward+' G'+boostInfo;
  updateBoosterUI();
}
...
function backToGarage(){
  ...
  updateResources();if(typeof updateQuestUI==='function')updateQuestUI();if(typeof updateBoosterUI==='function')updateBoosterUI();renderTree();renderCarousel();
}
