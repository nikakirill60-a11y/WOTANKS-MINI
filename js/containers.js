...
const COLLECTION_DB = {
  ...
  SU76I: { n: "СУ76И", nat: "ussr", tier: 3, hp: 400, dmg: 110, s: 0.8, isPT: true, off: 8, vr: 300, camo: 0.4, cls: 'td', armor: 60, nc: '#ff4444', premium: true, collection: true, desc: "Редчайшая ПТ-САУ." },
  ...
  PZ2J: { n: "Pz.II J", nat: "germany", tier: 3, hp: 450, dmg: 15, s: 0.7, mag: 6, reload: 3000, off: 5, vr: 260, camo: 0.3, cls: 'lt', armor: 80, nc: '#cc0000', premium: true, collection: true, desc: "Неубиваемый лёгкий танк." },
...
function renderContainerGrid() {
  const g = document.getElementById('container-grid'); if (!g) return; g.innerHTML = '';
...
function renderInventoryGrid() {
  const g = document.getElementById('inventory-grid'); if (!g) return;
  const empty = document.getElementById('inventory-empty'); if (!empty) return;
  g.innerHTML = '';
...
function renderCollectionGrid() {
  const g = document.getElementById('collection-grid'); if (!g) return; g.innerHTML = '';
