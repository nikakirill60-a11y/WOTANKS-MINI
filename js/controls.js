...
function setupMobileControls(){
  ...
  function hJ(touch){
    ...
    const dist=Math.hypot(dx,dy),max=r.width*.4;
    if(dist>max){dx=dx/dist*max;dy=dy/dist*max;}
    jS.style.transform='translate(calc(-50% + '+dx+'px), calc(-50% + '+dy+'px))';
    GameState.joystickData.dx=dx/max;
    GameState.joystickData.dy=dy/max;
    GameState.joystickData.mag=Math.min(dist/max,1);
