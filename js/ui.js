function crewMsg(msg,color){
  ...
  el.innerText=msg;el.style.color=color||'#fff';el.style.opacity=1;
...
function dmgLog(msg,color){
  ...
  d.className='dmg-msg';d.style.color=color||'#fff';d.innerText=msg;
  el.appendChild(d);setTimeout(function(){d.remove();},2000);
}
...
function exchangeXP(){
  if(GameState.XP>=100){
    GameState.XP-=100;GameState.GOLD+=10;
    document.getElementById('exchange-msg').innerText="✓";
    setTimeout(function(){document.getElementById('exchange-msg').innerText="";},1500);
    updateResources();
  }
}

function setNat(nat,btn){
  GameState.curNat=nat;renderTree();
...
function useCons(i){
  ...
  var el=document.getElementById('cons'+(i+1));if(el)el.classList.add('used');
  var mel=document.getElementById('mcons'+(i+1));if(mel)mel.classList.add('used');
  switch(i){
    ...
    case 4:GameState.paiokActive=true;crewMsg("Доп.паёк! +5% урон","#f39c12");snd('hit');break;
...
function buyBooster(type){
  ...
  updateResources();updateBoosterUI();saveProgress();
}
...
function equipModule(modId,slot){
  ...
  updateResources();showModules();saveProgress();
}
...
function showUpgrades(){
  ...
  var cats=[
    ...
    {key:'engine',icon:'🔧',title:'Двигатель',items:CONFIG.UPGRADES.engine},
    {key:'turret',icon:'🏰',title:'Башня',items:CONFIG.UPGRADES.turret}
  ];

  var html='<div class="upgrade-panel">';
  html+='<h3>🔩 Прокачка: '+t.n+' ['+CONFIG.TIER_ROMAN[tier]+']</h3>';
...
function updateQuestUI(){
  ...
  if(!q)return;
  if(q.claimed){document.getElementById('quest-widget').style.display='none';return;}
...
function claimQuestReward(){
  if(!GameState.quest23.claimed){
    ...
    updateResources();renderCarousel();updateQuestUI();saveProgress();
  }
}
...
function activatePromo(){
  ...
  if(GameState.usedPromos.indexOf(code)!==-1){
    result.innerText="Уже использован!";result.style.color="#e74c3c";return;
  }
  ...
  if(reward){
    ...
    if(reward.tank){
      if(GameState.owned.indexOf(reward.tank)===-1){
        GameState.owned.push(reward.tank);msg+='Танк: '+DB[reward.tank].n;
    ...
    result.innerText="Успешно! "+msg;result.style.color="#2ecc71";
    updateResources();renderCarousel();updateBoosterUI();saveProgress();
  }else{
    result.innerText="Неверный код!";result.style.color="#e74c3c";
  }
}

// === ПРОДАЖА ===
function sellTank(tid){
  if(GameState.owned.length<=1){alert("Нельзя продать последний!");return;}
  var t=DB[tid];
  ...
  if(confirm('Продать '+t.n+' за '+price+'₽?')){
    ...
    updateResources();renderCarousel();renderTree();
  }
}

// === ТРЕНИРОВКА ===
function fillTrainNatSelect(){
  var s=document.getElementById('train-nat-select');s.innerHTML='';
  var nats=[];
  ...
  for(var j=0;j<nats.length;j++){
    ...
    o.value=nats[j];o.innerText=CONFIG.NATIONS[nats[j]]||nats[j];
    s.appendChild(o);
  }
}
...
function fillEnemySelect(){
  ...
  var s=document.getElementById('enemy-select');s.innerHTML='';
  ...
  for(var i=0;i<ids.length;i++){
    ...
    o.innerText=DB[id].n+' ['+CONFIG.TIER_ROMAN[DB[id].tier]+']';
    s.appendChild(o);
  }
}
...
function renderCarousel(){
  var c=document.getElementById('tank-carousel');c.innerHTML='';
  ...
  for(var si=0;si<sorted.length;si++){
    ...
    var slot=document.createElement('div');slot.className='tank-slot';
    ...
    (function(tankId){
      slot.onclick=function(e){
        ...
        GameState.selected=tankId;renderCarousel();renderTree();
      };
    })(id);

    var sb=document.createElement('button');sb.className='sell-btn';sb.innerText='×';
    (function(tankId){sb.onclick=function(e){e.stopPropagation();sellTank(tankId);};})(id);
    slot.appendChild(sb);
    ...
    if(upg){
      ...
      if(total>0){
        var ui2=document.createElement('div');ui2.className='upg-indicator';ui2.innerText='🔩'+total;slot.appendChild(ui2);
      }
    }
    ...
    if(modCount>0){
      var mi=document.createElement('div');mi.className='mod-indicator';mi.innerText='⚙'+modCount;slot.appendChild(mi);
    }

    var wrap=document.createElement('div');wrap.className='slot-bg';
    var cv=document.createElement('canvas');cv.width=140;cv.height=80;
    drawTankIcon(cv,id);wrap.appendChild(cv);

    var info=document.createElement('div');info.className='slot-info';
    var tierEl=document.createElement('span');tierEl.className='slot-tier';tierEl.innerText=CONFIG.TIER_ROMAN[t.tier]||'XI';
    var nameEl=document.createElement('span');nameEl.className='slot-name';
    ...
    var clsEl=document.createElement('span');clsEl.className='slot-cls';
    ...
    info.appendChild(tierEl);info.appendChild(nameEl);info.appendChild(clsEl);
    slot.appendChild(wrap);slot.appendChild(info);
    c.appendChild(slot);
  }
}
...
function drawTankIcon(canvas,tankId){
  ...
  var t=DB[tankId];if(!t)return;
  ctx2.clearRect(0,0,canvas.width,canvas.height);
  ctx2.save();ctx2.translate(canvas.width/2,canvas.height/2);ctx2.scale(0.8,0.8);
  var a=-Math.PI/6;var s=t.s||1;
  var bc=t.nc||'#666';
  ...
  ctx2.save();ctx2.rotate(a);ctx2.fillStyle=bc;
  ...
  ctx2.save();var off=(t.off||0)*s;
  ctx2.translate(Math.cos(a)*off,Math.sin(a)*off);ctx2.rotate(a);
  ctx2.fillStyle=bc;ctx2.filter='brightness(1.2)';
  var turSz=t.dualGun?14:10;
  ctx2.fillRect(-turSz*s,-turSz*s,turSz*2*s,turSz*2*s);
  ctx2.filter='none';

  if(t.flame){
    ctx2.fillStyle="#333";ctx2.fillRect(5*s,-5*s,25*s,10*s);
    ctx2.fillStyle="#ff4500";ctx2.fillRect(28*s,-4*s,6*s,8*s);
  }else if(t.dualGun){
    var gunLen=35*s;
    var gunW=2.5*s;
    var sp=4*s;
    ...
    ctx2.fillStyle="#111";ctx2.fillRect(5*s,-3*s,35*s,6*s);
  }
...
function renderTree(){
  ...
  var sc=nodeScale;
  nodes.innerHTML='';ctx3.clearRect(0,0,tc.width,tc.height);
  ctx3.strokeStyle="#444";ctx3.lineWidth=2*sc;

  for(var id in DB){
    var t=DB[id];
    if(t.nat!==GameState.curNat||t.collection)continue;
    var sx=t.x*sc;var sy=t.y*sc;
    if(t.p&&DB[t.p]&&!DB[t.p].collection){
      ctx3.beginPath();ctx3.moveTo(DB[t.p].x*sc+62*sc,DB[t.p].y*sc+24*sc);
      ctx3.lineTo(sx,sy+24*sc);ctx3.stroke();
    }
    ...
    (function(tankId,tankData){
      div.onclick=function(){
        if(GameState.owned.indexOf(tankId)!==-1){GameState.selected=tankId;}
          ...
          GameState.XP-=tankData.xp;GameState.owned.push(tankId);GameState.selected=tankId;
        }
