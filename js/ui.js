...
function useCons(i) {
  ...
  if (GameState.SILVER < cons.cost) {
    crewMsg("§¯§Ö§ä §ã§Ö§â§Ö§Ò§â§Ñ!", "#e74c3c");
    return;
  }
  ...
  switch (i) {
    ...
    case 2:
      ...
      crewMsg("§¡§Õ§â§Ö§ß§Ñ§Ý§Ú§ß!", "#f39c12");
      snd('hit');
      break;
    case 3:
      ...
      crewMsg("§°§Ô§à§ß§î §á§à§ä§å§ê§Ö§ß!", "#2ecc71");
      snd('hit');
      break;
    case 4:
      GameState.paiokActive = true;
      crewMsg("§¥§à§á.§á§Ñ§×§Ü! +5% §å§â§à§ß", "#f39c12");
      snd('hit');
      break;
    case 5:
      GameState.fuelBoostActive = true;
      GameState.fuelBoostTimer = Date.now() + 15000;
      crewMsg("§´§à§á§Ý§Ú§Ó§à 105!", "#f39c12");
      snd('hit');
      break;
  }
}
...
function updateBoosterUI() {
  ...
  for (var ti = 0; ti < types.length; ti++) {
    ...
    if (active) {
      ...
      html += '<span style="color:#666">§Ó§í§Ü§Ý</span>';
    }
...
function showModules() {
  ...
  for (var i = 0; i < 3; i++) {
    ...
    if (mod) {
      ...
      html += '<span class="mod-empty">+ §³§Ý§à§ä ' + (i + 1) + '</span>';
    }
    html += '</div>';
  }
...
function selectModule(slot) {
  ...
  if (mods[slot]) {
    html += '<div class="mod-option" onclick="unequipModule(' + slot + ')"><span style="color:#e74c3c">? §³§ß§ñ§ä§î</span></div>';
  }
...
function showUpgrades() {
  ...
  html += '<h3>?? §±§â§à§Ü§Ñ§é§Ü§Ñ: ' + t.n + ' [' + (CONFIG.TIER_ROMAN[tier] || 'XI') + ']</h3>';
  ...
  for (var ci = 0; ci < cats.length; ci++) {
    ...
    for (var lvl = 0; lvl < cat.items.length; lvl++) {
      ...
      if (isEquipped) {
        html += '<div class="upg-btn installed">§µ§³§´§¡§¯§°§£§­§¦§¯§°</div>';
      } else if (isBought) {
        html += '<div class="upg-btn equip" onclick="equipUpgrade(\'' + tid + '\',\'' + cat.key + '\',' + lvl + ')">§µ§³§´§¡§¯§°§£§ª§´§¾</div>';
      } else if (canBuy && canAfford) {
        html += '<div class="upg-btn buy" onclick="buyUpgrade(\'' + tid + '\',\'' + cat.key + '\',' + lvl + ',' + cost + ')">§¬§µ§±§ª§´§¾ ' + cost + '?</div>';
      } else if (canBuy && !canAfford) {
        html += '<div class="upg-btn buy no-money">§¯§µ§¨§¯§° ' + cost + '?</div>';
      } else {
  ...
  html += '</div>';
  html += '<button class="btn btn-close" style="margin-top:15px" onclick="hideUpgrades()">§©§¡§¬§²§½§´§¾</button>';
  html += '</div>';
...
function buyUpgrade(tid, cat, lvl, cost) {
  if (GameState.SILVER < cost) {
    alert("§¯§Ö§Õ§à§ã§ä§Ñ§ä§à§é§ß§à §ã§Ö§â§Ö§Ò§â§Ñ!");
    return;
  }
  initTankUpgrades(tid);
...
function activatePromo() {
  ...
  if (GameState.usedPromos.indexOf(code) !== -1) {
    if (result) {
      result.innerText = "§µ§Ø§Ö §Ú§ã§á§à§Ý§î§Ù§à§Ó§Ñ§ß!";
  ...
  if (reward) {
    ...
    if (result) {
      result.innerText = "§µ§ã§á§Ö§ê§ß§à! " + msg;
    ...
    if (result) {
      result.innerText = "§¯§Ö§Ó§Ö§â§ß§í§Û §Ü§à§Õ!";
      result.style.color = "#e74c3c";
    }
  }
}
...
function sellTank(tid) {
  if (GameState.owned.length <= 1) {
    alert("§¯§Ö§Ý§î§Ù§ñ §á§â§à§Õ§Ñ§ä§î §á§à§ã§Ý§Ö§Õ§ß§Ú§Û §ä§Ñ§ß§Ü!");
    return;
  }
  var t = DB[tid];
...
async function loadLeaderboard() {
  ...
  if (!supabaseClient) {
    ...
    if (listEl) listEl.innerHTML = '<div style="padding:20px;text-align:center;color:#888;">?? §¯§Ö§ä §ã§à§Ö§Õ§Ú§ß§Ö§ß§Ú§ñ §ã §ã§Ö§â§Ó§Ö§â§à§Þ</div>';
    return;
  }
    ...
    html += `
      <div class="lb-row${isYou}">
        ...
        <div class="lb-row-name">
          ...
          <span>${user.username}${isYou ? ' (§£§í)' : ''}</span>
        </div>
...
async function loadOnlinePlayers() {
  ...
  if (!supabaseClient) {
    ...
    if (emptyEl) {
      emptyEl.innerHTML = '<div style="padding:20px;text-align:center;color:#888;">?? §°§ß§Ý§Ñ§Û§ß §Ú§Ô§â§à§Ü§Ú §ß§Ö§Õ§à§ã§ä§å§á§ß§í</div>';
